...
log = logging.getLogger("ncins.sign.tests")
...
def settings():
    s = get_settings()
    log.info("API endpoint: %s", s.create_operation_url)
    log.info(
...
@pytest.fixture
def client(settings) -> SignCreateOperationClient:
    return SignCreateOperationClient(settings=settings)
